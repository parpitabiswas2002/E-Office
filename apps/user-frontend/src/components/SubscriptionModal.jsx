...
export default function SubscriptionModal({ user, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
      {/* Modal Container */}
      <div
        className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh] animate-slideUp"
        role="dialog"
        aria-modal="true"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 bg-white sticky top-0 z-10">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Unlock premium drafting capabilities.
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            {plans.map((plan) => {
              ...
              return (
                ...
                >
                  ...
                  {plan.badge && (
                    <span
                      className={`absolute -top-2.5 right-4 text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-full ${
                        plan.id === "annual"
